...
async function main() {
  // Default options with very small imbalance
  const options: Options = {
    pairToManipulate: "DAI-USDC",
    exchange: "sushiswap",
    imbalancePercent: 2,  // Very small 2% imbalance
  ...
  // SAFER APPROACH: Directly manipulate token balances without changing K
  
  // 1. Find token storage slots
  const token0Slot = await findTokenBalanceSlot(token0.address, deployer.address);
  const token1Slot = await findTokenBalanceSlot(token1.address, deployer.address);
  ...
  // 2. Send both tokens to the pair and manipulate the relative amounts
  
  // Calculate the amounts - for small imbalances
  ...
  // Transfer tokens to pair with imbalance
  if (options.direction === "token0") {
    ...
    await token0.transfer(pairAddress, imbalanceAmount0);
    ...
    await token1.transfer(pairAddress, imbalanceAmount1);
  ...
  // Check price difference between exchanges
